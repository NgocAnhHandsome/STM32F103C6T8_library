...
void LCD_Init(void)
{
  ...
  LCD_Command(0x38); // giao thuc 4 bit, hien thi 2 hang, ki tu 5x8
...
{
	GPIO_WriteBit(GPIOA,LCD_D0, data & 1);
	GPIO_WriteBit(GPIOA,LCD_D1, ( data >> 1) & 1);
	GPIO_WriteBit(GPIOA,LCD_D2, ( data >> 2) & 1);
	GPIO_WriteBit(GPIOA,LCD_D3, ( data >> 3) & 1);
	GPIO_WriteBit(GPIOA,LCD_D4, ( data >> 4) & 1);
	GPIO_WriteBit(GPIOA,LCD_D5, ( data >> 5) & 1);
	GPIO_WriteBit(GPIOA,LCD_D6, ( data >> 6) & 1);
	GPIO_WriteBit(GPIOA,LCD_D7, ( data >> 7) & 1);
	LCD_Enable();
}
